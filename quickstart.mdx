...
<AccordionGroup>
  ...
  <Accordion icon="rectangle-terminal" title="Preview changes">
    ...
    1. Install the
    [Mintlify CLI](https://www.npmjs.com/package/mint) to preview the
    documentation changes locally with this command: ``` npm i -g mint ```
    2. Run the following command at the root of your documentation (where
    `docs.json` is): ``` mint dev ```
    <Note>
      If you’re currently using the legacy ```mint.json``` configuration file, please update the Mintlify CLI:


      ```npm i -g mint```
      And run the update command in your docs repository:
